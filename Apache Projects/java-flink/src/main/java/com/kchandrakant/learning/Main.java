...
import com.kchandrakant.learning.aggregate.CustomerAggregatorByCountry;
...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		FlinkKafkaConsumer<String> kafkaSource = new FlinkKafkaConsumer<>("customer.create", new SimpleStringSchema(),
				properties);
		...
		KeyedStream<Customer, String> customerPerCountryStream = stream.map(data -> {
			try {
				...
				LOG.info(String.format("exception reading data: %s", data)	);
				return null;
			}
