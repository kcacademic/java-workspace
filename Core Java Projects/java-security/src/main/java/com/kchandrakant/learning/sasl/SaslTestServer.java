...
/**
 * A sample server application that uses SASL to authenticate clients using
...
public class SaslTestServer {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 2) {
			System.err.println("Usage: java <options> SaslTestServer <service> <host>");
			System.exit(-1);
		}

		PrivilegedExceptionAction action = new SaslServerAction(args[0], args[1], PORT);
	...
	static class SaslServerAction implements PrivilegedExceptionAction {
		...
		public Object run() throws Exception {
			...
			while (loopCount++ < LOOP_LIMIT) {
				...
				SaslServer srv = Sasl.createSaslServer(MECH, service, serverName, props, cbh);

				if (srv == null) {
					throw new Exception("Unable to find server implementation for " + MECH);
				}
				...
				while (!srv.isComplete()) {
					try {
						...
						if (srv.isComplete()) {
							...
							clientMsg = conn.send(AppConnection.AUTH_INPROGRESS, challenge);
				...
				System.arraycopy(nowBytes, 0, reply, realMsg.length + 1, nowBytes.length);
