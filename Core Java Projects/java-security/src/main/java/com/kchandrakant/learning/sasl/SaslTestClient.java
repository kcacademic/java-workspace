package com.kchandrakant.learning.sasl;
...
/**
 * A sample client application that uses SASL to authenticate to a server using
 * Kerberos as the underlying mechanism. It then exchanges data securely with
 * the server.
 *
 * This sample program uses a ficticious application-level protocol. Every
 * message exchanged between the client and server an 8-byte header that
 * consists of two integers: the first integer represesents the
 * application-level command or status code while the second integer indicates
 * the length of the SASL buffer. This header is followed by the SASL buffer.
 *
 * The protocol is: 1. Authentication a. client sends initial response to server
 * containing authentication information b. server accepts and evaluates
 * response to generate challenge; it sends the challenge to the server. c.
 * client evaluates challenge to generate response; it sends the response; d.
 * Steps b and c are repeated until authentication succeeds or fails. 2. client
 * sends an encrypted message to the server. 3. server decryptes the message and
 * sends an encrypted one back that contains the original message plus the
 * current time.
 *
...
public class SaslTestClient {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 2) {
			System.err.println("Usage: java <options> SaslTestClient <service> <serverName>");
			System.exit(-1);
		}

		PrivilegedExceptionAction action = new SaslClientAction(args[0], args[1], PORT);
	...
	static class SaslClientAction implements PrivilegedExceptionAction {
		...
		public Object run() throws Exception {
			...
			SaslClient clnt = Sasl.createSaslClient(new String[] { MECH }, null, service, serverName, props, cbh);

			if (clnt == null) {
				throw new Exception("Unable to find client implementation for " + MECH);
			}
			...
			while (!clnt.isComplete() && (reply.getStatus() == AppConnection.AUTH_INPROGRESS
			...
			if (clnt.isComplete() && reply.getStatus() == AppConnection.SUCCESS) {
				...
				throw new Exception("Authentication failed: " + " connection status? " + reply.getStatus());
			}
			...
			if (reply.getStatus() == AppConnection.SUCCESS) {
				...
				byte[] clearReply = (sl ? clnt.unwrap(encryptedReply, 0, encryptedReply.length) : encryptedReply);
