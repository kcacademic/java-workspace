package com.sapient.learning.spnego;
 ...
 * .... 2. client sends a wrapped token to the server. 3. server sends a wrapped
 * token back to the client for the application
 *
...
public class GssSpNegoClient {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 2) {
			System.err.println("Usage: java <options> GssSpNegoClient <service> <serverName>");
			System.exit(-1);
		}
	...
	static class GssClientAction implements PrivilegedExceptionAction {
		...
		public Object run() throws Exception {
			...
			/*
			 * This Oid is used to represent the SPNEGO GSS-API mechanism. It is defined in
			 * RFC 2478. We will use this Oid whenever we need to indicate to the GSS-API
			 * that it must use SPNEGO for some purpose.
			 */
			...
			while (!context.isEstablished()) {
				...
				if (token != null) {
					if (verbose) {
						...
						System.out.println("writing token = " + getHexBytes(token));
					}
				...
				if (!context.isEstablished()) {
					...
					if (verbose) {
						System.out.println("reading token = " + getHexBytes(token));
						System.out.println(
			...
			/*
			 * Encrypt the data and send it across. Integrity protection is always applied,
			 * irrespective of confidentiality (i.e., encryption). You can use the same
			 * token (byte array) as that used when establishing the context.
			 */
			...
			/*
			 * Now we will allow the server to decrypt the message, append a time/date on
			 * it, and send then it back.
			 */
